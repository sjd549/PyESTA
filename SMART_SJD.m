...
ProjectName = 'S1-000017';		%Define global project name
...
I_Sol_Equil=-400;			        %-400;          %-520;          %-350;
I_Sol_EndEquil=-375;                %-375;          %-500;          %-325;
...
I_Div1_Equil=+0525;					%+0525;         %-1500/1800;    %+1000/1800;    (HIGH FOR +delta, LOW FOR -delta, before efit)
...
GridCells_R = 300;          %[Cells]    (300)   (>300 would be good, causes RAM issues)     (-delta:325)
...
%Plot plasma current over ramp timescale (TauR)
figure('units','inch','position',[10 10 12 12]);
subplot(2,1,1)
plot(time_adaptive*1000, Ip_output/1000, 'LineWidth',2)
title(gca,'SMART Plasma Current iter(0)');
...
xlabel(gca,'Time (ms)');
ylabel(gca,'Plasma Current I_{p} (kA)');
set(gca,'XLim',[time(3)*1e3, time(5)*1e3]);
set(gca, 'FontSize', 13, 'LineWidth', 0.75);
%%%%%
subplot(2,1,2)
plot(time_adaptive(1:end-1)*1000, Delta_Ip_output/1000, 'LineWidth',2)
...
set(gca, 'FontSize', 13, 'LineWidth', 0.75);
%%%%%
Filename = '_PlasmaCurrent_Closeup';
saveas(gcf, strcat(ProjectName,Filename,FigExt));
...
fileID=fopen(Filename,'w');
fprintf(fileID,'%s %s %s %s\r\n', 'Vloop [V]', '       Vloop_Lc [V]', '    Eloop [V/m]', '   Eloop_eff [V/m]');
fprintf(fileID,'%1.12f %1.12f %1.12f %1.12f\r\n', Vloop_Passive', Vloop_Lc_Passive, Eloop_Passive', Eloop_eff_Passive');
...
function [Vloop,Eloop]=LoopVoltage(CoilWaveforms,time,RSol,ZMaxSol,ZMinSol,RGeo)
    ...
    dIdt = max(dIdt_Array);                 %Maximum Sol current ramp [A/t]
