...
ShotName = 'S3-000006-B';		%Define shot name: typically Sx-xxxxxx"
...
copyfile('SMART_SJD_Phase3.m',SimDir)
...
Te = 700;			% Electron Temperature [eV]                     Gusev2013 Figure 12 suggests 1keV < Te < 2keV, 2keV < Ti < 5keV
Ti = Te*0.50;		% Ion Temperature      [eV]                     for 0.3MW < Pnbi < 1.1MW with 18keV < Einj < 30keV and Ip = 500kA
...
Kappa = 2.00;		% Elongation           [-] (~1.70 --> 2.00)
delta = 0.20;		% Triangularity        [-] (~0.20)
Z_eff = 2.00;       % Effective Charge     [C] (~1.44 - 2.00)
li2 = 1;			% Inductance	       [-] (??????)

%Compute further operating conditions (primarily used for Topeol2)
Gr_Frac = 0.50;                            % Greenwald Fraction       [-]       ~1.2e20 m-3 for Ip=400kA, GlobusM2 is approx 0.7e20 m-3
Gr_Limit = 1e20*(Ip*1e-6/(pi*rGeo^2));     % Greenwald Limit          [m-3]
ne = abs(Gr_Limit*Gr_Frac);                % Electron Density         [m-3]     ~0.8e20 m-3 for rGeo at 0.700
...
Kappa_efit = 2.20;					% Elongation			[-] (+1.70 -> +2.00)  :: (Zmax-Zmin)/2rGeo
...
%%%%%%%                                                       %Kappa=1.80   %Kappa=1.80     %Kappa=1.80,delta=-0.5
                                    %TauR1=60ms  %TauR=60ms   %TauR=60ms    %TauR=60ms      %TauR=60ms
%Max +-15000/18000 A at 300A/ms     %RGeo=0.42   %RGeo=0.42   %RGeo=0.46    %RGeo=0.46      %RGeo=0.42
%Solenoid coil currents [A]         %Phase3      %Phase3-d    %Phase3+d     %SingleNull+d   %SingleNull-d           
I_Sol_Null=+16000;					%+16000;     %+16000;     %+18000;      %+15000;        %+15000;
I_Sol_MidRamp=+00000;				%+00000;     %+00000;     %+00000;      %+00000;        %+00000;
I_Sol_Equil=-03000;                 %-03000;     %-04500;     %-04500;      %-06000;        %-04000;
I_Sol_EndEquil=-10000;           	%-10000;     %-11000;     %-12000;      %-12500;        %-11000;

%Max +-8000 A at 300A/ms
%PF & Div Equilibrium coil currents [A] @time(4,5,6)                                        
I_PF1_Equil=-4000;					%-4000;      %-4000;      %-4000        %-4000;         %-4000;
I_PF2_Equil=-4000;					%-4000;      %-4000;      %-4000        %-4000;         %-4000;
I_Div1_Equil=+2500;					%+2500;      %-5000;      %+5000;       %+3000;         %-7000;       %(Pos for +d, Neg for -d)
I_Div2_Equil=+0000;                 %+0000;      %+0000;      %+0000;       %+0000;         %+0000;
...
TauR1 = 0.060;			% Breakdown Ramp Timescale  [s] Determines max loop voltage
